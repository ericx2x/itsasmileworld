 ...
 * @uses custom_php()->db->get_table_name() to dynamically retrieve the snippet table name
 *
...
function get_snippets( array $ids = array(), $multisite = null ) {
	...
	foreach ( $snippets as $index => $snippet ) {
		...
		$snippets[ $index ] = new MyPHPCode( $snippet );
	}

	return apply_filters( 'custom_php/get_snippets', $snippets, $multisite );
}
...
function get_all_snippet_tags() {
	...
	foreach ( $all_tags as $snippet_tags ) {
		$snippet_tags = custom_php_build_tags_array( $snippet_tags );
 ...
 */
function custom_php_build_tags_array( $tags ) {
 ...
 *
 * @return MyPHPCode                 A single snippet object
 */
function get_snippet( $id = 0, $multisite = null ) {
	...
	$id = absint( $id );
	$table = custom_php()->db->get_table_name( $multisite );
	...
	return apply_filters( 'custom_php/get_snippet', $snippet, $id, $multisite );
}
...
function activate_snippet( $id, $multisite = null ) {
	...
	do_action( 'custom_php/activate_snippet', $id, $multisite );
}
...
function deactivate_snippet( $id, $multisite = null ) {
	...
	do_action( 'custom_php/deactivate_snippet', $id, $multisite );
}
 ...
 * @uses custom_php()->db->get_table_name() to dynamically retrieve the name of the snippet table
 *
...
function delete_snippet( $id, $multisite = null ) {
	...
	$wpdb->delete(
		custom_php()->db->get_table_name( $multisite ),
		array( 'id' => $id ),
		array( '%d' )
	);

	do_action( 'custom_php/delete_snippet', $id, $multisite );
}
 ...
 * @uses custom_php()->db->get_table_name() To dynamically retrieve the name of the snippet table
 *
 * @param  MyPHPCode $snippet   The snippet to add/update to the database
 *
 * @return int                  The ID of the snippet
 */
function save_snippet(MyPHPCode $snippet ) {
	/** @var wpdb $wpdb */
	global $wpdb;

	$table = custom_php()->db->get_table_name( $snippet->network );
	...
	if ( 0 == $snippet->id ) {
		...
		do_action( 'custom_php/create_snippet', $snippet->id, $table );
	} else {

		/* Otherwise update the snippet data */
		$wpdb->update( $table, $data, array( 'id' => $snippet->id ), null, array( '%d' ) );

		do_action( 'custom_php/update_snippet', $snippet->id, $table );
	}
...
function update_snippet_fields( $snippet_id, $fields, $network = null ) {
	...
	$table = custom_php()->db->get_table_name( $network );
...
function execute_snippet( $code, $id = 0, $catch_output = true ) {
	...
	do_action( 'custom_php/after_execute_snippet', $id, $code, $result );
...
function execute_active_snippets() {
	...
	$sql_format = "SELECT id, code, scope, code_type FROM %s WHERE scope IN ('global', 'single-use', %%s) ";
	...
	foreach ( $queries as $table_name => $query ) {
		...
		foreach ( $active_snippets as $snippet ) {
            ...
            if ( apply_filters( 'custom_php/allow_execute_snippet', true, $snippet_id, $table_name ) ) {
                if($snippet['code_type'] == 'default') {
