 ...
 * @package MyCustomPhp
 */
class MyCustomPhp_List_Table extends WP_List_Table {
	...
	public function __construct() {
		...
		$status = apply_filters( 'custom_php/list_table/default_view', 'all' );
		...
		add_screen_option( 'per_page', array(
			'label'   => __( 'Snippets per page', 'my-custom-php' ),
			'default' => 999,
		...
		foreach ( $filters as $filter ) {
			add_filter( 'custom_php/list_table/column_description', $filter );
		}
	 ...
	 *
	 * @param MyPHPCode $snippet The snippet used for the current row
		...
		switch ( $column_name ) {
			...
			case 'description':
				return empty( $snippet->desc ) ? '&#8212;' :
					apply_filters( 'custom_php/list_table/column_description', $snippet->desc );
			default:
				return apply_filters( "custom_php/list_table/column_{$column_name}", $snippet );
		}
	}
	...
	public function get_action_link( $action, $snippet, $escape = true ) {
		...
		if ( 'edit' === $action ) {
			return custom_php()->get_snippet_edit_url( $snippet->id, $network_redirect ? 'network' : 'self' );
		}

		$query_args = array( 'action' => $action, 'id' => $snippet->id );

		$url = $network_redirect ? add_query_arg( $query_args, custom_php()->get_menu_url( 'manage', 'network' ) ) : add_query_arg( $query_args );
	 ...
	 *
	 * @param MyPHPCode $snippet The current snippet
	 *
	 * @return array The action links HTML
	 */
	private function get_snippet_action_links(MyPHPCode $snippet ) {
		$actions = array();
		...
		if ( ! $this->is_network && $snippet->network && ! $snippet->shared_network ) {
			...
			if ( $snippet->active ) {
				$actions['network_active'] = esc_html__( 'Network Active', 'my-custom-php' );
			} else {
				$actions['network_only'] = esc_html__( 'Network Only', 'my-custom-php' );
			}
		} elseif ( ! $snippet->shared_network || current_user_can( custom_php()->get_network_cap_name() ) ) {
			...
			$simple_actions = array(
				'edit'   => esc_html__( 'Edit', 'my-custom-php' ),
				'clone'  => esc_html__( 'Clone', 'my-custom-php' ),
				'export' => esc_html__( 'Export', 'my-custom-php' ),
			);
			...
			$actions['delete'] = sprintf(
				...
				esc_html__( 'Delete', 'my-custom-php' ),
				$this->get_action_link( 'delete', $snippet ),
				esc_js( sprintf(
					'return confirm("%s");',
					esc_html__( 'You are about to permanently delete the selected item.', 'my-custom-php' ) . "\n" .
					esc_html__( "'Cancel' to stop, 'OK' to delete.", 'my-custom-php' )
				) )
			);
		}

		return apply_filters( 'custom_php/list_table/row_actions', $actions );
	}
	...
	protected function get_activation_switch( $snippet ) {
		...
		if ( $snippet->active ) {
			$actions['deactivate'] = sprintf(
				$link_format,
				$snippet->network ? esc_html__( 'Network Deactivate', 'my-custom-php' ) : esc_html__( 'Deactivate', 'my-custom-php' ),
				$this->get_action_link( 'deactivate', $snippet )
			);
		} elseif ( 'single-use' === $snippet->scope ) {

			$actions['run_once'] = sprintf( $link_format, esc_html__( 'Run Once', 'my-custom-php' ), $this->get_action_link( 'run-once', $snippet ) );

		} else {
			$actions['activate'] = sprintf(
				$link_format,
				$snippet->network ? esc_html__( 'Network Activate', 'my-custom-php' ) : esc_html__( 'Activate', 'my-custom-php' ),
	 ...
	 *
	 * @param MyPHPCode $snippet The snippet being used for the current row
	 *
	 * @return string The content of the column to output
	 */
	protected function column_name( $snippet ) {
		...
		$title = empty( $snippet->name ) ? sprintf( __( 'Untitled #%d', 'my-custom-php' ), $snippet->id ) : $snippet->name;

		$row_actions = $this->row_actions(
			$this->get_snippet_action_links( $snippet ),
			apply_filters( 'custom_php/list_table/row_actions_always_visible', true )
		);
		...
		if ( $this->is_network || ! $snippet->network || current_user_can( custom_php()->get_network_cap_name() ) ) {

			$out = sprintf(
				'<a href="%s" class="snippet-name">%s</a>',
				custom_php()->get_snippet_edit_url( $snippet->id, $snippet->network ? 'network' : 'admin' ),
				$out
			);
		}

		if ( $snippet->shared_network ) {
			$out .= ' <span class="badge">' . esc_html__( 'Shared on Network', 'my-custom-php' ) . '</span>';
		}

		/* Return the name contents */

		$out = apply_filters( 'custom_php/list_table/column_name', $out, $snippet );
	...
	protected function column_cb( $snippet ) {
		...
		return apply_filters( 'custom_php/list_table/column_cb', $out, $snippet );
	}
	...
	public function get_columns() {
		$columns = array(
			...
			'name'        => __( 'Name', 'my-custom-php' ),
			'id'          => __( 'ID', 'my-custom-php' ),
			'description' => __( 'Description', 'my-custom-php' ),
			'tags'        => __( 'Tags', 'my-custom-php' ),
			'priority'    => __( 'Priority', 'my-custom-php' ),
		);
		...
		return apply_filters( 'custom_php/list_table/columns', $columns );
	}
	...
	public function get_sortable_columns() {
		...
		return apply_filters( 'custom_php/list_table/sortable_columns', $sortable_columns );
	}
	...
	public function get_bulk_actions() {
		$actions = array(
			'activate-selected'   => $this->is_network ? __( 'Network Activate', 'my-custom-php' ) : __( 'Activate', 'my-custom-php' ),
			'deactivate-selected' => $this->is_network ? __( 'Network Deactivate', 'my-custom-php' ) : __( 'Deactivate', 'my-custom-php' ),
			'clone-selected'      => __( 'Clone', 'my-custom-php' ),
			'download-selected'   => __( 'Download', 'my-custom-php' ),
			'export-selected'     => __( 'Export', 'my-custom-php' ),
			'delete-selected'     => __( 'Delete', 'my-custom-php' ),
		);

		return apply_filters( 'custom_php/list_table/bulk_actions', $actions );
	}
	...
	public function get_table_classes() {
		...
		return apply_filters( 'custom_php/list_table/table_classes', $classes );
	}
	...
	public function get_views() {
		...
		return apply_filters( 'custom_php/list_table/views', $status_links );
	}
	...
	public function extra_tablenav( $which ) {
		...
		if ( 'recently_activated' === $status ) {
			submit_button( __( 'Clear List', 'my-custom-php' ), 'secondary', 'clear-recent-list', false );
		}

		do_action( 'custom_php/list_table/actions', $which );
	...
	public function required_form_fields( $context = 'main' ) {
		...
		$vars = apply_filters(
			'custom_php/list_table/required_form_fields',
		...
		do_action( 'custom_php/list_table/print_required_form_fields', $context );
	}
	...
	public function no_items() {
		esc_html_e( "It looks like you don't have any snippets.", 'my-custom-php' );
		printf(
			' <a href="%s">%s</a>',
			esc_url( custom_php()->get_menu_url( 'add' ) ),
			esc_html__( 'Perhaps you would like to add a new one?', 'my-custom-php' )
		);
	}
	...
	public function prepare_items() {
		...
		/* Fetch all snippets */
		$snippets['all'] = apply_filters( 'custom_php/list_table/get_snippets', get_snippets( array() ) );
	 ...
	 * @ignore
	 *
	 * @param MyPHPCode $a The first snippet to compare
	 * @param MyPHPCode $b The second snippet to compare
	 *
	...
	private function usort_reorder_callback( $a, $b ) {
		...
		! empty( $_REQUEST['orderby'] )
			? $_REQUEST['orderby']
			: apply_filters( 'custom_php/list_table/default_orderby', 'priority' )
		);
		...
		! empty( $_REQUEST['order'] )
			? $_REQUEST['order']
			: apply_filters( 'custom_php/list_table/default_order', 'asc' )
		);
	 ...
	 * @ignore
	 *
	 * @param MyPHPCode $snippet The snippet being filtered
	 *
	...
	public function search_notice() {
		if ( ! empty( $_REQUEST['s'] ) || ! empty( $_GET['tag'] ) ) {
			...
			echo '<span class="subtitle">' . __( 'Search results', 'my-custom-php' );

			if ( ! empty( $_REQUEST['s'] ) ) {
				/* translators: %s: search query */
				echo sprintf( __( ' for &ldquo;%s&rdquo;', 'my-custom-php' ), esc_html( $_REQUEST['s'] ) );
			}

			if ( ! empty( $_GET['tag'] ) ) {
				/* translators: %s: tag name */
				echo sprintf( __( ' in tag &ldquo;%s&rdquo;', 'my-custom-php' ), esc_html( $_GET['tag'] ) );
			}
			...
			printf(
				...
				__( 'Clear Filters', 'my-custom-php' )
			);
		}
	}
