 ...
 * @return array
 */
function _custom_php_save_imported_snippets( $snippets, $multisite = null, $dup_action = 'ignore' ) {
...
function import_snippets_json( $file, $multisite = null, $dup_action = 'ignore' ) {
	...
	do_action( 'custom_php/import/json', $file, $multisite );
...
function import_snippets_xml( $file, $multisite = null, $dup_action = 'ignore' ) {
	...
	do_action( 'custom_php/import/xml', $file, $multisite );
 ...
 * @return array
 */
function custom_php_prepare_export( $format, $ids, $table_name = '', $mime_type = '' ) {
	global $wpdb;

	/* Fetch the snippets from the database */
	if ( '' === $table_name ) {
		$table_name = custom_php()->db->get_table_name();
	}
	...
	if ( 1 == count( $ids ) ) {
		...
		$first_snippet = new MyPHPCode( $snippets[0] );
	...
	$filename = apply_filters( 'custom_php/export/filename', $filename, $title );
...
function download_snippets( $ids, $table_name = '' ) {
	$snippets = custom_php_prepare_export( 'php', $ids, $table_name );
...
function export_snippets( $ids, $table_name = '' ) {
	$raw_snippets = custom_php_prepare_export( 'json', $ids, $table_name, 'application/json' );
	$final_snippets = array();

	foreach ( $raw_snippets as $snippet ) {
		$snippet = new MyPHPCode( $snippet );

		$fields = array( 'name', 'desc', 'tags', 'scope', 'code', 'code_type' );
		$final_snippet = array();
	...
	$data = array(
		'generator'    => 'Code Snippets v' . MY_CUSTOM_PHP_VERSION,
		'date_created' => date( 'Y-m-d H:i' ),
		'snippets'     => $final_snippets,
	);

	echo wp_json_encode( $data, apply_filters( 'custom_php/export/json_encode_options', 0 ) );
	exit;
}
