...
class Import
{
     ...
     * @return array
     */
    static function _custom_php_save_imported_snippets( $snippets, $multisite = null, $dup_action = 'ignore' ) {
     ...
     */
    static function import_snippets_json( $file, $multisite = null, $dup_action = 'ignore' ) {
        ...
        $imported = _custom_php_save_imported_snippets( $snippets, $multisite, $dup_action );
     ...
     * @return array|bool An array of imported snippet IDs on success, false on failure
     */
    static function import_snippets_xml( $file, $multisite = null, $dup_action = 'ignore' ) {
        ...
        $imported = _custom_php_save_imported_snippets( $snippets, $dup_action, $multisite );
    ...
    static function prepare_export( $format, $ids, $table_name = '', $mime_type = '' ) {
        global $wpdb;

        if ( count( $ids ) ) {
            $snippets = DatabaseManager::getInstance()->getSnippetByID($ids);
        ...
        $filename = "{$title}.my-custom-php.{$format}";
        $filename = apply_filters( 'mycc/export/filename', $filename, $title );
    ...
    static function download_snippets( $ids, $table_name = '' ) {
        $snippets = self::prepare_export( 'php', $ids, $table_name );
    ...
    static function export_snippets( $ids, $table_name = '' ) {
        $raw_snippets = self::prepare_export( 'json', $ids, $table_name, 'application/json' );
        $final_snippets = array();

        foreach ( $raw_snippets as $snippet ) {

            $fields = Snippet::fieldList();
            // exclude fields
        ...
        $data = array(
            'generator'    => 'Code Snippets v' . MYCC_VERSION,
            'date_created' => date( 'Y-m-d H:i' ),
            'snippets'     => $final_snippets,
        );

        echo wp_json_encode( $data, apply_filters( 'mycc/export/json_encode_options', 0 ) );
        exit;
    }
